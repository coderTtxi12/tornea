...
/**
 * Tabla de equipos del dashboard (plantillas / `MyLeaguesTeamRow`).
 * Está pensada solo para este caso de uso; si más adelante necesitás una tabla
 * genérica con otras columnas, conviene extraer un componente nuevo en lugar de
 * forzar abstracciones aquí.
 */
...
type SortState = { key: "name" | "players"; dir: "asc" | "desc" };
...
export type TeamsFilterableTableHandle = {
  /** Quita filtros de columnas, restablece orden (Equipo A→Z) y cierra el panel de filtro. */
...
/**
 * Tabla de equipos con ordenación (nombre / jugadores), filtros por columna en portal
 * y chips de etiquetas activas bajo el título de cada columna filtrable.
 */
export const TeamsFilterableTable = forwardRef<TeamsFilterableTableHandle, TeamsFilterableTableProps>(
  ...
  const hasActiveColumnFilters =
    filterLeague.size + filterCategory.size + filterStatus.size + filterCode.size > 0;
  ...
  useEffect(() => {
    ...
    function onDocMouseDown(e: MouseEvent) {
      ...
      if ((e.target as Element | null)?.closest?.(`[${TEAMS_FILTERABLE_TABLE_TRIGGER_ATTR}]`))
        return;
      closeFilter();
    }
  ...
  function headerFilterColumn(col: FilterColumn, label: string, selected: Set<string>) {
    ...
    return (
      <button
        type="button"
        {...{ [TEAMS_FILTERABLE_TABLE_TRIGGER_ATTR]: "" }}
        ...
        <table className="w-full min-w-[52rem] text-left text-sm">
          <thead>
            <tr className="text-foreground-muted border-border border-b text-[11px] font-bold tracking-wide uppercase">
              ...
              <th className="max-w-[14rem] px-4 py-3 align-top whitespace-normal">
                {headerFilterColumn("category", "Categoría", filterCategory)}
                {selectedTagsChips(filterCategory, setFilterCategory, "Categoría")}
              </th>
              <th className="px-4 py-3 align-top">
                <button
                  type="button"
              ...
              <th className="max-w-[14rem] px-4 py-3 align-top whitespace-normal">
                ...
                {selectedTagsChips(filterStatus, setFilterStatus, "Estado")}
              </th>
              <th className="max-w-[14rem] px-4 py-3 align-top whitespace-normal">
                {headerFilterColumn("code", "Código", filterCode)}
                {selectedTagsChips(filterCode, setFilterCode, "Código")}
              </th>
              <th className="px-4 py-3 align-top" />
            </tr>
          </thead>
                ...
                <tr
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      onEditTeam({ leagueId: t.leagueId, teamId: t.id });
                    }
                  }}
                >
                  <td className="px-4 py-2.5 font-medium">{t.name}</td>
                  <td className="text-foreground-muted px-4 py-2.5">{t.leagueName}</td>
                  <td className="text-foreground-muted px-4 py-2.5">{t.categoryName ?? "—"}</td>
                  <td className="text-foreground-muted px-4 py-2.5 tabular-nums">{t.playersCount}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <MockBadge tone={t.status === "active" ? "lime" : "muted"}>{badgeCode(t)}</MockBadge>
                  </td>
                  <td
...
TeamsFilterableTable.displayName = "TeamsFilterableTable";
